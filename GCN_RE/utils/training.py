...
def train_and_save(dataset, saving_dir, data_name, epochs, bucket_size, args):
    ...
    print("---make them into mini-batch---")
    buckets = bin_data_into_buckets(sentences, bucket_size)
    gcn_model = GCN_RE.GCN_model.Train(maxlength=maxlength, args = args)
    ...
    for i in range(epochs):
        ...
        for bucket in random_buckets:
            # try:
            loss = 0
            bucket_data = aux.get_data_from_sentences(bucket, maxlength)
            ...
            if len(bucket_data) >= 1:
                correct, loss = gcn_model.train(data = bucket_data)
                # if cnt_batch % 1 == 0:
            import time
            now = time.strftime("%H:%M:%S")
            correct_num += correct
            all_num += len(bucket_data)
